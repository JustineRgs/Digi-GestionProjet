...
class Security {
    public static function isConnected() {
        if (session_status()!==2) { // Condition -> Si le statut de session n'est pas égale à 2, alors on session start
            session_start();
        }
    ...
    public static function ConnectUser() {
        ...
        if (count($searchUser) >= 1){
            ...
            if ($user === $searchUser[0]->getMail() && password_verify($pwd,$searchUser[0]->getPwd())) { // Vérification de la ressemblance des mdp et mail
